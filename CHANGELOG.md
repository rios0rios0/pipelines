...
- added a scheduled **release reconciliation** safety net for the "bumped but never released" failure mode: a bump PR merges to `main` but that `main` run fails the quality gate, so `delivery-release` never runs and no tag/release is cut even though `CHANGELOG.md` already carries the version. New reusable workflow `.github/workflows/release-reconcile.yaml` (opt-in via a scheduled consumer — see `.docs/examples/github-go-docker/.github/workflows/reconcile.yaml`) runs `global/scripts/shared/reconcile-releases.sh`, which diffs the released `CHANGELOG.md` versions against the git tags and resolves each gap to its bump commit, then (re-)pushes the missing tags so the tag-push delivery path re-cuts the releases and opens/updates a tracking issue. Covered by `.github/tests/test-release-reconcile.sh` (wired into `make test`)
...
- changed `github/global/stages/40-delivery/release` and the `go-library`, `composer-library`, and `maven-library` reusable workflows to also cut a release on a **version-tag push**, not only on a `main`-branch bump merge. The gate jobs already skip on tags (`if: "!startsWith(github.ref, 'refs/tags/')"`), so a bump whose `main` run failed the quality gate can now be recovered by simply (re-)pushing its version tag: the release action derives the version straight from the tag ref instead of the commit message. `go-binary` already delivered on tag pushes via GoReleaser, so it is unchanged, and the `main`-branch bump path is untouched
