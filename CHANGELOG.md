...
- fixed the terra CLI never updating on persistent, self-hosted CI runners. The shared "install terra" step used by all three platforms (`azure-devops/terra/abstracts/terra.yaml`, `gitlab/terra/abstracts/terra.yaml`, and `.github/workflows/terra.yaml`) runs `curl -fsSL .../install.sh | sh` followed by `terra install`, but [`install.sh`](https://github.com/rios0rios0/terra/blob/main/install.sh) refuses to overwrite an existing binary without `--force`: when terra is already present it prints "already installed", exits 0, and performs no version comparison at all. On ephemeral, cloud-hosted agents this is harmless because every job starts from a clean image and installs the latest release; on a long-lived self-hosted runner the binary persists across jobs, so the pipeline silently keeps whatever terra version was first installed and ignores every release published afterwards — including the Terraform and Terragrunt version bumps a newer terra would pull. Added `terra self-update --force` (terra's own non-interactive updater, which fetches the same public GitHub releases) immediately before `terra install`, so the CLI is brought to the latest release on every run and only then resolves the matching Terraform and Terragrunt versions
