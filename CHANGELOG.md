...
- added `PRE_STEPS` parameter to `azure-devops/terraform/terra.yaml`, the `20-security` stage (forwarded to the `sast:semgrep` and `sast:trivy` jobs), and the `30-test` stage it composes, mirroring the hook already accepted by the `terra/` template family used by customer-clusters / shared-toolbox / central-clusters. Steps passed via `PRE_STEPS` are spliced into both the SAST scanners that parse `*.tf` files and the `test:smoke` / `test:e2e` jobs before `terraform init` runs, so consumers whose modules pin private-git submodules (e.g. `k8s-deployment` references `git@dev.azure.com-arancia:v3/ZestSecurity/terraform-modules/tf-container-image?ref=1.0.1`) can ship their SSH setup steps once and have both the security scans and tests resolve `terraform init` cleanly. Without this hook the init aborts with `ssh: Could not resolve hostname dev.azure.com-arancia`. Caught when `k8s-deployment` `!11632` failed CI for a different reason than the rest of the first batch and traced to a missing host alias on the agent.
...
- added `azure-devops/terraform/stages/30-test/terra.yaml` — a new `test` stage slotted between `20-security` and `35-management` in the Azure DevOps `terraform` pipeline template used by the `terraform-modules` project. Runs **three test tiers** against the single module at the repo root, each as an opt-in signal so modules onboard incrementally:
    - **Tier 0 (`test:smoke`)** — plan-time `terraform test` against `tests/*.tftest.hcl` with `mock_provider` blocks. No network, no credentials, no cluster. Publishes `junit-terra-tests.xml`.
    - **Tier 1 (`test:e2e`, apply-time HCL)** — `terraform test -test-directory=tests/e2e` against `tests/e2e/*.tftest.hcl` using `command = apply` and real providers. Same language as smoke, no Go toolchain needed. Assertions limited to `output` values and `expect_failures`. Publishes `junit-e2e-tftest.xml`.
    - **Tier 2 (`test:e2e`, apply-time Go)** — the shared `terratest/run.sh` runner pointed at `tests/terratest/`. Arbitrary Go with `client-go` / `helm-sdk` for rich post-apply assertions ("pod is Running", "service has endpoints", "secret has expected base64 shape"). Publishes `junit-e2e-terratest.xml`.

    Tiers 1+2 share a single disposable [kind](https://kind.sigs.k8s.io/) cluster per build (scoped by `$(Build.BuildId)` so concurrent pipeline runs on the same agent pool don't collide). The cluster is torn down with `condition: always()` so a red test still cleans up; both `KUBECONFIG` (for `kubectl` / `client-go` / terratest) and `KUBE_CONFIG_PATH` (which the Terraform `kubernetes` / `helm` providers read — they do NOT fall back to `KUBECONFIG`, verified the hard way) are exported. Go is only installed when the terratest tier is detected, saving ~20s per build on modules that stop at Tiers 0+1. Tier 0 runs in its own job in parallel with Tier 1+2, so plan-time feedback lands in under a minute even while kind is provisioning.

    The `kind` binary is downloaded into a job-local directory (`Agent.TempDirectory/bin`, prepended to `PATH`) and verified against the upstream `.sha256sum` before execution — a tampered mirror or corrupted download is caught before the binary runs, and passwordless `sudo` is no longer required so the template works on locked-down self-hosted pools. Architecture is detected at runtime (`amd64` / `arm64`) so the template runs on Linux ARM agent pools. The cluster's Kubernetes version is pinned via `--image kindest/node:v1.31.0` so e2e runs stay reproducible across kind releases. Tier 2 captures the `terratest/run.sh` exit code before renaming `junit-terratest.xml → junit-e2e-terratest.xml` so a red test still publishes the JUnit instead of `set -e` aborting the rename and surfacing as "no test results". Tier 2 opt-in additionally requires `tests/terratest/go.mod` (the runner does `go test ./...` from that directory) and surfaces a clear "missing `go.mod`" log line instead of failing mid-stage. The `kubectl get nodes` post-provision verification is best-effort — it's guarded with `command -v kubectl` and skipped with a clear log line on agents that don't have `kubectl` pre-installed, since the e2e tests themselves talk to the API via terratest / Terraform providers that bring their own clients.
