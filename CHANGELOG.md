...
- added `azure-devops/terraform/abstracts/setup-ssh-modules.yaml` — a reusable PRE_STEP abstract that downloads an Azure DevOps Secure File holding an SSH private key, drops it at `~/.ssh/id_rsa`, writes a `~/.ssh/config` aliasing the consumer-supplied SSH host (default `dev.azure.com-arancia`) to `ssh.dev.azure.com`, starts `ssh-agent`, and smoke-tests the alias. Mirrors `azure-devops/terra/abstracts/setup-modules-repo.yaml` from the Zest-private `terra/` family used by `customer-clusters` / `central-clusters` / `shared-toolbox`, so single-tier `terraform-modules/*` consumers (e.g. `k8s-deployment` referencing `git@dev.azure.com-arancia:v3/ZestSecurity/terraform-modules/tf-container-image?ref=1.0.1`) lift the same one-liner setup instead of duplicating the SSH boilerplate per repo. Parameterized by `SSH_PRIVATE_KEY_SECURE_FILE` (default `azure_devops_ssh_private_key`) and `SSH_HOST_ALIAS` so non-Zest consumers can opt in with their own secure file and host alias
