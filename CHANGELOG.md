...
- added `azure-devops/terraform/stages/45-e2e/terra.yaml` — an opt-in apply-time e2e stage for the terraform-modules pipeline. Provisions a single-node [kind](https://kind.sigs.k8s.io/) cluster per build (scoped by `$(Build.BuildId)` so concurrent pipeline runs on the same agent pool don't collide), exports both `KUBECONFIG` (for `kubectl`) and `KUBE_CONFIG_PATH` (which the Terraform `kubernetes` and `helm` providers read — they do NOT fall back to `KUBECONFIG`, verified the hard way), invokes `terraform test -test-directory=tests/e2e` so the e2e suite uses the same native `.tftest.hcl` framework already driving `smoke.tftest.hcl`, and tears the cluster down with `condition: always()` even on red tests. The stage short-circuits cleanly when `tests/e2e/*.tftest.hcl` is absent — only modules that opt in by creating `tests/e2e/` pay the stage runtime. Publishes JUnit at `build/reports/junit-e2e.xml` and the whole reports directory as the `terraform-e2e-reports` pipeline artifact. Slotted after `40-delivery` so a failing e2e on a merged `chore(bump)` commit doesn't block the tag (the apply-time signal is a soft gate for now; can be bumped ahead of delivery once the suite is mature). Wired into `azure-devops/terraform/terra.yaml` as the sixth stage. No Go toolchain required on the agent — earlier drafts of this stage used the terratest Go runner, but the native `terraform test` framework covers apply/destroy without a second language in the stack.
