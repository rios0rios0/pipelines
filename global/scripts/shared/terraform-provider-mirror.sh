...
EOF
  ...
  # The fallback's store list, which is the primary's MINUS the active plugin
  # cache. That subtraction is the whole point: the fallback runs with
  # `TF_PLUGIN_CACHE_DIR` in force, and Terraform cannot install a package it
  # selected from a mirror into a cache directory that IS that mirror --
  #
  #   Error while installing hashicorp/http v3.6.0: cannot install existing
  #   provider directory <cache>/registry.terraform.io/hashicorp/http/3.6.0/
  #   linux_amd64 to itself
  #
  # -- and, when the two are merely different directories, it writes a symlink
  # into the cache that a later direct install cannot overwrite. Both are the same
  # rule seen from two angles: a directory may be an active cache or a mirror
  ...
  provider_mirror_emit_mirrors() {
    printf '%s\n' "$1" | while IFS= read -r provider_mirror_dir; do
      [ -n "${provider_mirror_dir}" ] || continue
...

  {
    ...
    provider_mirror_emit_mirrors "${provider_mirror_candidates}"
    echo '  direct {'
  ...
  {
    echo '# Generated by global/scripts/shared/terraform-provider-mirror.sh.'
    echo '# Fallback: the registry may also serve, and the plugin cache is NOT listed'
    echo '# here -- it is the write target for this attempt and cannot also be a source.'
    echo 'provider_installation {'
    provider_mirror_emit_mirrors "${provider_mirror_fallback_candidates}"
    echo '  direct {}'
    echo '}'
...
terraform_init_cached() {
  ...
  # The fallback keeps the OTHER mirrors in play and adds the registry to them, so
  # a provider one of those stores holds still installs locally and github is asked
  # solely for the genuine misses. `TF_PLUGIN_CACHE_DIR` stays in force here and is
  # deliberately absent from this config's store list (see
  # `provider_mirror_fallback_candidates`); it is the write target, which is what
  # makes a cold machine converge -- the downloaded misses land in it as real
  # directories, and the PRIMARY attempt does list it, so the next directory
  # needing that version is served locally with no request at all.
  ...
  while [ "${terraform_init_attempt}" -le "${TF_INIT_MAX_ATTEMPTS:-4}" ]; do
    ...
    if [ "${terraform_init_attempt}" -lt "${TF_INIT_MAX_ATTEMPTS:-4}" ]; then
      ...
      echo "  terraform init failed (attempt ${terraform_init_attempt}/${TF_INIT_MAX_ATTEMPTS:-4}); retrying in ${terraform_init_sleep}s. A transient registry.terraform.io 5xx or a github.com 503 on provider checksums clears on retry; anything repeating identically across every attempt is deterministic and the retries will not help — read the error above rather than this line." >&2
