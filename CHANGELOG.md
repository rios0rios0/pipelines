...
- added `.github/tests/test-tftest-gen.sh` — 21 BDD-style assertions (given / when / then) that exercise `gen_smoke_tests.py` against synthetic `terraform-modules` layouts: minimal module, required var with `validation {}`, `object(...)`-typed validated var (regression guard for the `_invalid_stub` type-error bug), `--force` override behaviour against hand-written tests, optional (defaulted) var skipped from validation-coverage, and `run.sh` shebang + POSIX compliance. Wired into `make test` so any future edit that re-introduces the bash-ism in `run.sh`, the `os.system` shell-interpolation, or the `{}`-for-objects invalid stub is caught before merge.
