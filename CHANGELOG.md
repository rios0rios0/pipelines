...
- fixed `.github/workflows/update-major-version-tag.yaml` leaving the rolling major-version GitHub Release (e.g., `v4`) with a stale `publishedAt` and a concatenated `body`. `softprops/action-gh-release@v2` cannot refresh `publishedAt` on an existing release and was appending another `generate_release_notes` block on every bump, so `v4` showed below the latest SemVer release in the GitHub Releases UI and accumulated multiple "What's Changed" sections. The workflow now deletes the existing major-version release with `gh release delete --yes` (without `--cleanup-tag`, so the force-pushed tag is preserved) before recreating it, refreshing `publishedAt` and giving it a single clean changelog body each release.
