...
- fixed `release.yaml` "Create Tag" step crashing with `RELEASE_VERSION: unbound variable` on non-bump merges whose commit message contains single quotes. The `SKIP_RELEASE` guard failed because Azure DevOps variable propagation appended a trailing quote to the value when the commit body contained apostrophes (e.g., `customer's`), making `[ "true'" = "true" ]` evaluate to false and falling through to the unbound variable. Removed `set -u` from the tag-creation script and added `:-` fallback on `RELEASE_VERSION` references as defense-in-depth
