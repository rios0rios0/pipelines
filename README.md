...
2. **Changelog validation** — every branch carries its own changelog entry. Which form is required depends on the repository's layout, and the check decides by looking for a `.chlog.yaml` at the root:
   - **[chlog](https://github.com/luizjhonata/chlog) repositories** (a `.chlog.yaml` exists) — an ordinary branch must **add a new fragment** under `.changes/unreleased/`, and the failure message quotes the `chlog new -k <Kind> -b "<description>"` that writes one. A `bump/*` or `chore/bump-*` branch carries no fragment — `chlog merge` has already folded the pending ones into `CHANGELOG.md` — so on those the requirement flips to an updated `CHANGELOG.md`.
