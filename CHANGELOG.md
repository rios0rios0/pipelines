...
- added Android NDK Clang as C cross-compiler for Android targets in GoReleaser binary delivery via `nttld/setup-ndk@v1`
...
- changed Go cross-compile Android targets to use Android NDK Clang as C cross-compiler (`CGO_ENABLED=1`) instead of Zig, fixing missing bionic libc headers
...
- changed GoReleaser template to use Android NDK Clang `overrides` for Android targets with `CGO_ENABLED=1`, fixing Android binary builds
...
- removed Zig dependency from Go cross-compile check and GoReleaser delivery in favor of `CGO_ENABLED=0` for type-checking and Android NDK Clang for binary builds
...
- fixed Android cross-compilation by replacing Zig (which lacks Android bionic libc headers) with Android NDK Clang for GoReleaser binary delivery
