...
- fixed `azure-devops/javascript/stages/30-tests/yarn.yaml` `test_e2e` job failing during `Prepare job` with "Could not find a pool with name `$(SELF_HOSTED_POOL)`" on every consumer that does not define a self-hosted pool. The `SELF_HOSTED_POOL` parameter default was `"$(SELF_HOSTED_POOL)"` — a runtime variable reference that is never an empty string at compile time, so the `${{ if ne(parameters.SELF_HOSTED_POOL, '') }}` guard always emitted the `pool` block and Azure DevOps then failed at job prep because the variable was undefined. `Prepare job` runs before `continueOnError` takes effect, so the phase was marked `failed` and the `tests` stage's `failed` result cascaded — `management`, `delivery`, and `release` were all skipped. The default is now an empty string so the guard correctly omits the `pool` block and the job inherits the pipeline's default pool
- fixed `azure-devops/javascript/yarn-docker.yaml` not forwarding `WORKING_DIRECTORY` to the `30-tests`, `35-management`, and `40-delivery` stages. The `test-all.yaml` step composed `testResultsFiles` and `summaryFileLocation` as `${{ WORKING_DIRECTORY }}/junit.xml` and `${{ WORKING_DIRECTORY }}/cobertura.xml`; with `WORKING_DIRECTORY` defaulting to empty those became absolute paths `/junit.xml` and `/cobertura.xml`, so `PublishTestResults@2` could not find the artefacts and `PublishCodeCoverageResults@2` with `failIfCoverageEmpty: true` hard-failed. `WORKING_DIRECTORY` is now forwarded to all three downstream stages as `$(System.DefaultWorkingDirectory)`, matching the pattern already used for `10-code-check`
