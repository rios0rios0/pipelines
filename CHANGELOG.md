...
- fixed `corepack enable` failing outright on Node 25 and later, which no longer ship Corepack: every Yarn job in `yarn.yaml` and the `yarn` branch of `javascript/stages/10-code-check/knip` now installs it first with `npm i -g corepack@latest --force`. Corepack was removed from the Node distribution rather than deprecated in place, so the step does not warn and degrade -- the shim is simply absent and the job dies on its second step, before a single dependency is installed, which is what a consumer would have hit the moment they pointed the new `node_version` input at a supported major. The `--force` is what makes the same line safe on the versions that do still bundle Corepack, where npm otherwise refuses to overwrite the existing `corepack` shim with `EEXIST` and would break exactly the consumers the change was meant to leave alone
