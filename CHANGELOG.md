...
- fixed the Trivy `convert` step still panicking with the nil-URL `SIGSEGV` from `pkg/report/sarif.go:103` even after the JSON/SARIF split (PR #348). The same bug is reached from `trivy convert` because it shares the `SarifWriter` code path. `global/scripts/tools/trivy/run.sh` now pre-processes `trivy.json` with `jq`'s `walk` builtin, rewriting every `git@host:path` string to the equivalent valid RFC 3986 URL `ssh://git@host/path` before `trivy convert` runs. The conversion succeeds silently, `trivy.sarif` is produced, and the stack trace no longer floods the build logs. The fallback `|| echo "SARIF conversion failed; trivy.json is authoritative."` is kept as a safety net for unrelated convert failures.
