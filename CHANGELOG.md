...
- added `/build/` to a new `.gitignore` at the repo root so `make test` inside this pipelines repo (the validation suite in `.github/tests/`) doesn't leave untracked report artifacts after a local run. Previously the repo had no root `.gitignore` (only `.idea/.gitignore`), and the new standardized `build/reports/` output would otherwise show up under `git status` on every test run
...
- changed `global/scripts/languages/golang/test/run.sh` to write every test-report artifact under `${REPORT_PATH:-build/reports}/` instead of the repo root. The runner previously dropped `junit.xml`, `junit-unit.xml`, `junit-integration.xml`, `coverage.txt`, `unit_coverage.txt`, `integration_coverage.txt`, `coverage.xml`, and `cobertura.xml` at the caller's working directory, which across ~300 consumer repos caused those files to regularly slip into commits because a `.gitignore` covering every filename variant doesn't scale (`cobertura.xml` vs `coverage.xml` vs `junit-report.xml` etc.). The runner now follows the Terraform tier-runner idiom already used by `global/scripts/languages/terraform/terratest/run.sh` — a single `REPORT_PATH="${REPORT_PATH:-build/reports}"` + `mkdir -p` at the top, every output bound to a local variable, and the cleanup trap removes intermediates from the new location. Downstream repos can now ignore a single `build/` directory instead of enumerating report filenames
- changed `.github/workflows/go.yaml`, `azure-devops/golang/abstracts/test.yaml`, and `gitlab/golang/stages/30-tests/go.yaml` to publish test results from `build/reports/` to match the relocated runner output. Covers the `dorny/test-reporter` path + `actions/upload-artifact` paths on GitHub, the `PublishTestResults@2` / `PublishCodeCoverageResults@2` / `PublishPipelineArtifact@1` tasks on Azure DevOps, and the `artifacts.paths` + `reports.coverage_report.path` entries on GitLab
- changed `makefiles/golang.mk` and `makefiles/python.mk` `REPORT_PATH` default from `./reports` to `build/reports`. The Go makefile's previous value was never honored (the runner ignored the env var and wrote to the repo root anyway), and the Python value was inconsistent with `azure-devops/python/stages/30-tests/pdm.yaml` which already expected `build/reports/junit.xml` + `build/reports/cobertura.xml`. This repo doesn't own a Python test runner (`pdm run test` is a downstream `pyproject.toml` concern), so the makefile default is the full extent of what can be standardized upstream — consumers must point pytest at `--junit-xml=${REPORT_PATH}/junit.xml --cov-report=xml:${REPORT_PATH}/cobertura.xml`
- changed `azure-devops/javascript/stages/30-tests/steps/test-all.yaml` parameter defaults `JUNIT_RELATIVE_PATH` and `COBERTURA_RELATIVE_PATH` from `junit.xml` / `cobertura.xml` to `build/reports/junit.xml` / `build/reports/cobertura.xml`, and hard-coded `azure-devops/javascript/stages/30-tests/steps/test-e2e.yaml` `testResultsFiles` + `summaryFileLocation` under `build/reports/` too. Consumers relying on the defaults must configure Jest/Vitest/Cypress to write under `build/reports/` (e.g., `jest --coverageDirectory=build/reports --outputFile=build/reports/junit.xml`); the template can only publish what the downstream `package.json` produces, but the default contract now aligns with every other language
- changed `.github/workflows/yarn.yaml` and `.github/workflows/npm.yaml` `dorny/test-reporter` + `hashFiles` paths from `junit-report.xml` to `build/reports/junit-report.xml` to match the new convention. The existing `coverage/` directory upload stays untouched — that's a separate Vitest artifact tree, not part of the root-pollution filenames the user flagged
- changed `azure-devops/dotnet/stages/30-tests/core.yaml`, `gitlab/dotnet/stages/30-tests/sdk.yaml`, `gitlab/dotnet/stages/30-tests/powershell.yaml`, and `gitlab/dotnet/stages/30-tests/framework.yaml` to emit every coverage artifact under `build/reports/dotnet/` instead of the repo-root `TestResults/` directory. Adds `--ResultsDirectory:build/reports/dotnet` (Linux `dotnet vstest`) / `/ResultsDirectory:build\reports\dotnet` (Windows `vstest.console.exe`) so the `.coverage` / `.trx` intermediates land under the ignored tree; updates every follow-up step (`find ./build/reports/dotnet`, `dotnet CodeCoverage analyze /output:...`, `dotnet ReportGenerator -reports:... -targetdir:...`, `cat .../Cobertura.xml`) and the GitLab `artifacts.paths` + `reports.coverage_report.path` entries to read/write the new location. Previously a `TestResults/` directory appeared at the caller's repo root on every build, and consumers without the right `.gitignore` entry leaked the `.coverage` / `.trx` blobs into commits
- changed `global/scripts/tools/sonarqube/run.sh` Go coverage auto-detection loop to prepend `build/reports/coverage.txt` and `build/reports/coverage.out` to the search list, so SonarQube finds the new canonical Go coverage file. Existing entries for legacy locations (`coverage.out`, `coverage.txt`, `coverage/*.txt`, etc.) stay in place so consumers who haven't migrated still get coverage uploaded
...
- fixed `gitlab/golang/stages/30-tests/go.yaml` `reports.coverage_report.path` pointing at a file the Go runner never produced. The previous value `coverage.xml` matched an empty `touch coverage.xml` leftover in `global/scripts/languages/golang/test/run.sh` (nothing ever wrote to it — the actual Cobertura output goes to `cobertura.xml`), so the GitLab MR widget silently failed to render coverage for every Go project. Now points at `build/reports/cobertura.xml`, the canonical post-move location, and the stale `touch coverage.xml` line was removed from the runner in the same change
