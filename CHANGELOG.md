...
- fixed the Terragrunt install step in `azure-devops/terraform/abstracts/terra-terraform.yaml` failing with `./terragrunt: line 1016: syntax error: unexpected end of file`. The step resolved its download URL at runtime by `grep`-ing an **unauthenticated** call to `api.github.com/repos/gruntwork-io/terragrunt/releases/latest`, and nothing validated either the API response or the download. Two independent defects fell out of that: shared CI runners share an egress IP and routinely exhaust the 60-requests-per-hour anonymous limit, leaving the `grep` with no match and the command substitution empty so `curl` ran without a usable URL (the observed result was a ~72 kB text payload written to `terragrunt` -- a real binary is ~85 MB -- which `chmod +x` then executed as a shell script, producing that syntax error); and upstream now publishes `terragrunt_linux_amd64.tar.gz` and `terragrunt_linux_amd64.zip` next to the raw binary, so the glob matches **three** assets and expands to three URLs against a single `-o`. The version is now pinned via a new `TERRAGRUNT_VERSION` parameter (default `v1.1.1`) and downloaded from an exact URL, removing the GitHub API dependency entirely and making the toolchain reproducible -- matching how Terraform was already pinned, which is also now overridable via `TERRAFORM_VERSION`. The download uses `curl -f` so an HTTP error exits non-zero instead of writing the error page to the output file, retries transient failures, and asserts the result is an ELF binary before running it, so a future regression fails with a readable message rather than a shell syntax error
