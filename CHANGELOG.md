...
- replaced the sample Azure DevOps host alias, organization and project names used in the `azure-devops/terraform` template comments and in earlier changelog entries with neutral placeholders, so the examples no longer reference a real private organization
