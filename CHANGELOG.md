...
- added `azure-devops/terraform/stages/45-e2e/terra.yaml` — an opt-in apply-time e2e stage for the terraform-modules pipeline. Provisions a single-node [kind](https://kind.sigs.k8s.io/) cluster per build (scoped by `$(Build.BuildId)` so concurrent pipeline runs on the same agent pool don't collide), exports both `KUBECONFIG` (for `kubectl` / `client-go` / terratest) and `KUBE_CONFIG_PATH` (which the Terraform `kubernetes` and `helm` providers read — they do NOT fall back to `KUBECONFIG`, verified the hard way), runs **two complementary tiers sharing the same cluster**, and tears it down with `condition: always()` even on red tests. Tier 1 is `terraform test -test-directory=tests/e2e`, driving `.tftest.hcl` files with `command = apply` and real providers — same language as `smoke.tftest.hcl`, no Go toolchain needed, good for "apply must succeed, module must expose output X". Tier 2 is the shared `terratest/run.sh` runner pointed at `tests/terratest/` — Go + the terratest library with access to `client-go`, `helm-sdk`, etc., good for "pod is Running, service has endpoints, secret has expected base64 shape". Modules opt into whichever tier(s) fit their assertions; a module with neither directory no-ops the stage silently (there are 9+ `azm-*` / `clf-*` repos that can't apply against kind at all). Go is only installed when the terratest tier is detected, saving ~20s per build on modules that stop at Tier 1. Publishes JUnit at `build/reports/junit-e2e-tftest.xml` and `build/reports/junit-e2e-terratest.xml` plus the whole reports directory as the `terraform-e2e-reports` pipeline artifact. Slotted after `40-delivery` so a failing e2e on a merged `chore(bump)` commit doesn't block the tag (the apply-time signal is a soft gate for now; can be bumped ahead of delivery once the suite is mature). Wired into `azure-devops/terraform/terra.yaml` as the sixth stage.
...
- added `azure-devops/terraform/stages/30-test/terra.yaml` — a new `test` stage slotted between `20-security` and `35-management` in the Azure DevOps `terraform` pipeline template used by the `terraform-modules` project. Runs `terraform test -junit-xml=build/reports/junit-terra-tests.xml` against the single module at the repo root, publishes JUnit via `PublishTestResults@2` so the Tests tab surfaces per-run results, and publishes the whole `build/reports/` directory as a pipeline artifact. No-ops cleanly on repos without a `tests/` directory so 24 module repos onboard incrementally. Wired into `azure-devops/terraform/terra.yaml` as the fifth stage; skipped on tag builds where re-running tests gates nothing.
