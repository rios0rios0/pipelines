...
- fixed pipelines that reference `azure-devops/global/stages/20-security/docker.yaml@pipelines` failing to compile with `Could not find /azure-devops/global/stages/20-security/docker.yaml`. That file was renamed to `semgrep.yaml` (and its Gitleaks job extracted into a new `gitleaks.yaml`) in the earlier `refactor(sast): renamed docker-named SAST stage files to per-tool names`, which silently orphaned every consumer still on the old reference. Restored `docker.yaml` as a thin backward-compatibility bundle that re-composes the same two jobs it used to emit (`sast:semgrep` + `sast:gitleaks`) by delegating to the per-tool templates rather than duplicating them, so the pre-refactor reference works again while new pipelines are still expected to use the per-tool templates directly
