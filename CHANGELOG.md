...
- fixed `azure-devops/global/stages/40-delivery/release.yaml` `Create Tag` step to actually retry on transient HTTP statuses by replacing the non-existent `--retry-on-http-error 429,500,502,503,504` curl flag with a bash retry loop on the captured `HTTP_STATUS`. curl's built-in `--retry` only triggers when curl exits non-zero, which requires `--fail`, but `--fail` would also abort the script on `409` (tag already exists, idempotent success) before the existing `case` could recognize it — so the retry has to live in bash. Same five-attempt / five-second-delay behaviour as the original intent, just driven from the right place. Symptom that motivated this: Zest-App `kickstart` build 36253 emitted `curl: option --retry-on-http-error: is unknown` / `Bash exited with code '2'` even with the static curl 8.19.0 bootstrap in place, because no curl version recognises that flag
- fixed `azure-devops/global/stages/40-delivery/release.yaml` `Install Static Curl` step to stream-extract only the `curl` entry from the static-curl tarball via `tar -xJf ... -O curl > "$CURL_DIR/curl"` instead of writing tar entries to disk with `tar -xJf ... -C "$CURL_DIR"` before the SHA check. Previous code ran `tar` on the downloaded archive *before* any integrity check — so a malicious or corrupted tarball could exploit tar-metadata path-traversal (`..`-segments, absolute paths) or symlink tricks during extraction to write files outside `$CURL_DIR` even though the post-extract SHA verification of `$CURL_DIR/curl` would still detect tampering of the `curl` binary itself. `tar -O` (`--to-stdout`) bypasses the entry's claimed destination entirely: the runner reads only the byte stream of the named entry and we redirect it to a path we control. The SHA check still runs against `$CURL_DIR/curl` before `chmod +x`, so the binary that gets executed is still pinned. Net effect: tarball integrity gap closes without losing the existing extracted-binary checksum guarantee
- fixed `azure-devops/global/stages/40-delivery/release.yaml` `Install Static Curl` step to verify the SHA256 of the **extracted `curl` binary** instead of the `.tar.xz` tarball. `stunnel/static-curl`'s release body publishes per-arch SHA256 of the unpacked binary (not the wrapper archive), so the previous code was hashing the tarball with the binary's checksum and aborting every release pipeline with `sha256sum: WARNING: 1 computed checksum did NOT match` / `Bash exited with code '1'` — symptom seen on Zest-App `kickstart` build 36251. The step now extracts the tarball first and runs `sha256sum -c -` against `$CURL_DIR/curl`, matching the artefact upstream actually signs and the binary the subsequent steps execute. Tarball-level integrity is not maintained by `stunnel/static-curl`, so a tarball-level pin would be a self-signed assertion rather than a verifiable one
- fixed `azure-devops/global/stages/40-delivery/release.yaml` `Create Tag` step to bootstrap a SHA-pinned static `curl` `8.19.0` from `stunnel/static-curl` before the `annotatedtags` POST. Microsoft-hosted `ubuntu-24.04` (and `ubuntu-latest`, which currently maps to it) ship `curl` `8.5.0` — the `--retry-on-http-error` flag added in the previous fix landed in `curl` `8.9.0` (Jul-2024), so every release pipeline on Microsoft-hosted Ubuntu was failing with `curl: option --retry-on-http-error: is unknown` / `Bash exited with code '2'`. Symptom seen on Zest-App `kickstart` build `36210` (`1.28.0`) and every other consumer of this template since the retry-on-http-error change merged. The bootstrap downloads the linux glibc tarball matching `uname -m` (`x86_64` / `aarch64`), verifies its SHA256, extracts to a `mktemp -d`, and uses that binary for the API call only — the system `curl` is untouched, so other tasks linking `libcurl` (`git-lfs`, `az` CLI) are not affected. The download runs once per job in a dedicated `Install Static Curl` step that exposes the binary path through the `STATIC_CURL` pipeline variable, so multi-prefix `TAG_PREFIXES` (e.g. the common `['', 'v']` case) reuse the same binary instead of repeating the download + integrity check per prefix. Bump `CURL_VERSION` + the per-arch SHA256 together when rolling forward
