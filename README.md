...
Dart parser rather than the JUnit through its experimental `java-junit` one, because `tojunit`
records each suite's class name as the runner's absolute path with the separators turned into
dots, which that parser can neither read as a file nor show as a name. The file is the
